...
<div class="modal fade" id="edit_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLongTitle">Edit Keluar</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        ...
        <form id="form_edit" method="POST">
           @csrf
            ...
            <div class="row form-group">
              <div class="col col-md-3"><label>Deskripsi :</label></div>
...
$(document).ready(function(){
    ...
    $("#tb_keluar").on('click','.btn-success',function(){
      var data = tb_keluar.row( $(this).parents('tr') ).data();
      $("#edit_id").val(data.id_keluar)
      $("#tgl_edit").html(data.tgl_keluar);
    ...
    $("#tb_keluar").on('click','.btn-danger',function(){
      var data = tb_keluar.row( $(this).parents('tr') ).data();
      var t = confirm("Apakah anda akan menghapus pengeluaran "+data.jumlah+" ?");
      if (t) {
        var d = {"id":data.id_keluar};
        action_data(APP_URL+"/api/keluar/delete",d,key).done(function(resp){
          if (resp.success) {
            alert(resp.message);
    ...
    $("#form_edit").submit(function(e){
        ...
        btn.html('Update');
        btn.attr('disabled', true);
        action_data(APP_URL+"/keluar/edit", d, _key).done(function(resp){
                if (resp.success) {
                    alert(resp.message);
                    window.location.href = "{{ route('keluar')}}";
                }else{
